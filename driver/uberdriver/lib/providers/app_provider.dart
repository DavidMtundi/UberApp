...
import 'package:uberdriver/models/ride_Request.dart';
import 'package:uberdriver/models/rider.dart';
import 'package:uberdriver/models/route.dart';
import 'package:uberdriver/services/map_requests.dart';
import 'package:uberdriver/services/ride_request.dart';
import 'package:uberdriver/services/rider.dart';
...
class AppStateProvider with ChangeNotifier {
  ...
  RideRequestModel rideRequestModel = RideRequestModel(
    valueid: "20",
    valueusername: "TestDriver",
    valueuserId: "Userid",
    valuedestination: "Nairobi",
    valuedLatitude: 0,
    valuedLongitude: 0,
    valueuLatitude: 0,
    valueuLongitude: 0,
  );
  late RequestModelFirebase requestModelFirebase = RequestModelFirebase(
      valueid: "TestId",
      valueusername: "TestUser",
      valueuserId: "Testuserid",
      valuedriverId: "Userid",
      valuestatus: "RIDE");

  RiderModel riderModel = RiderModel(
      valueid: "valueid",
      valuename: "Rider",
      valueemail: "Rideremail",
      valuephone: "RiderPhone",
      valuetoken: "0",
      valuephoto: "assets/images/rider.png",
      valuevotes: 0,
      valuetrips: 0,
      valuerating: 0);
...
//    _subscribeUser();
  ...
  void sendRequest(
      {String? intendedLocation, required LatLng coordinates}) async {
...
//    requestModelFirebase = await _requestServices.getRequestById(id);
    ...
    requestStream = _requestServices.requestStream().listen((querySnapshot) {
      querySnapshot.docChanges.forEach((doc) {
        if (doc.doc.get(FieldPath(const ['id'])) == id) {
          requestModelFirebase = RequestModelFirebase.fromSnapshot(doc.doc);
          notifyListeners();
          switch (doc.doc.get(FieldPath(const ['status']))) {
            case CANCELLED:
              print("====== CANCELELD");
              break;
            case ACCEPTED:
  ...
  percentageCounter({String? requestId, required BuildContext context}) {
    notifyListeners();
